# 手順 1 : 作成する仮想マシン用のサブネットを作成

この手順では、既存の Azure 仮想ネットワークに移行する仮想マシン用のサブネットを作成します。移行する仮想マシンは、専用のサブネット内で動作する必要があります。
...
既存の Azure 仮想ネットワークに移行する仮想マシン用のサブネットを作成する具体的な手順は以下のとおりです。
...
3. 画面左に \[**サブネットの追加**\] ブレードが表示されるので各項目を以下のように設定します
    ...
    |NAT ゲートウェイ|`internet-gateway`を選択 (※)|
    |ネットワークセキュリティグループ|なし|
    |ルートテーブル|なし|
    |サービス|指定しない|
    |サブネットをサービスに委任|なし|
...
ここまでの手順で、既存の仮想ネットワークに移行する仮想マシン用のサブネットを追加する作業は完了です。

<br>

## 次へ

👉　[**手順 2 : Linux 仮想マシンを作成し、Jump Box マシンから SSH 接続で操作**](jp-ex02.md)
